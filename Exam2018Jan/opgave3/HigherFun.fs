    ...
    | EnumVal(name, field) -> 
      ...
      | EVal(_, fields) ->
        let res = 
          fields 
          |> List.indexed 
          |> List.tryFind (fun (_,v) -> v = field)
        if res.IsNone then failwith $"field %s{field} does not exist on enum %s{name}"
        else res.Value |> fst |> Int
      | e -> failwith $"cannot access field on value type %A{e}"
    | Enum(name, values, body) -> 
      let bodyEnv = (name, EVal(name, values)) :: env
      eval body bodyEnv
