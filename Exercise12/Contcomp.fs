...
let addIFZERO lab3 C =
  match C with
  | GOTO lab :: C1 -> IFNZRO lab :: C1
  | _ -> IFZERO lab3 :: C

let addIFNZRO lab3 C =
  match C with
  | GOTO lab :: C1 -> IFZERO lab :: C1
  | _ -> IFNZRO lab3 :: C
...
let rec addCST i C =
    ...
    | (_, CSTI i2:: SWAP ::LT::C1) -> 
      let res = if i2 < i then 1 else 0
      addCST res C1
    | (_, CSTI i2:: LT::C1) -> 
      let res = if i < i2 then 1 else 0
      addCST res C1
    | (_, CSTI i2:: EQ::C1) ->
      let res = if i = i2 then 1 else 0
      addCST res C1
