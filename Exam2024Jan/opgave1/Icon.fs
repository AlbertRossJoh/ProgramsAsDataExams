...
let rec eval (e : expr) (cont : cont) (econt : econt) = 
    ...
    | Fail -> econt ()
    | FromList(lst) -> 
      if List.length lst  = 0 then failwith "You cannot provide FromList with an empty list"
      else
        let rec loop lst =
          match lst with
          | [] -> econt ()
        ...
        loop lst
    | FromMergeList(xs, ys) -> 
      if List.length xs  = 0 || List.length ys = 0 then failwith "You cannot provide FromList with an empty list"
      else
